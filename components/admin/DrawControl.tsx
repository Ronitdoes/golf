...
interface DrawControlProps {
  drawId: string;
  savedSimulation?: SimulationData;
  savedLogicType?: 'random' | 'algorithmic';
}

export default function DrawControl({ drawId, savedSimulation, savedLogicType }: DrawControlProps) {
  const [logicType, setLogicType] = useState<'random' | 'algorithmic'>(savedLogicType ?? 'random');
  const [isSimulating, setIsSimulating] = useState(false);
  const [isPublishing, setIsPublishing] = useState(false);
  const [simulationData, setSimulationData] = useState<SimulationData | null>(savedSimulation ?? null);
    ...
    try {
      ...
      const res = await fetch('/api/draws/run', {
        ...
        body: JSON.stringify({ 
          ...
          mode: 'publish', 
          logicType,
          // Use the exact numbers from simulation — never re-draw on publish
          drawnNumbers: simulationData?.drawnNumbers ?? null
        })
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
  ...
  return (
    <div className="space-y-10">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                className={`flex flex-col items-start gap-1 px-6 py-4 rounded-2xl border-2 transition-all ${logicType === 'random' ? 'border-green-500 bg-green-500/10 text-white' : 'border-neutral-800 bg-neutral-950 text-neutral-500 hover:border-neutral-700'}`}
              >
                <div className="flex items-center gap-3 font-bold">
                  <div className={`w-4 h-4 rounded-full border-2 shrink-0 ${logicType === 'random' ? 'border-green-500 bg-green-500' : 'border-neutral-700'}`} />
                  Random
                </div>
                <span className="text-[10px] font-medium text-neutral-500 text-left pl-7 leading-tight">5 numbers drawn with equal probability from 1–45</span>
              </button>
              <button 
                onClick={() => setLogicType('algorithmic')}
                className={`flex flex-col items-start gap-1 px-6 py-4 rounded-2xl border-2 transition-all ${logicType === 'algorithmic' ? 'border-green-500 bg-green-500/10 text-white' : 'border-neutral-800 bg-neutral-950 text-neutral-500 hover:border-neutral-700'}`}
              >
                <div className="flex items-center gap-3 font-bold">
                  <div className={`w-4 h-4 rounded-full border-2 shrink-0 ${logicType === 'algorithmic' ? 'border-green-500 bg-green-500' : 'border-neutral-700'}`} />
                  Weighted
                </div>
                <span className="text-[10px] font-medium text-neutral-500 text-left pl-7 leading-tight">Numbers weighted by frequency of real player scores — fairer to active golfers</span>
              </button>
            </div>
          </div>
      ...
      {/* Confirmation Modal — offset left by sidebar width so it centers in content area */}
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center pl-[190px] p-6 bg-neutral-950/90 animate-in fade-in duration-300">
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl p-10 w-full max-w-lg shadow-[0_50px_100px_-20px_rgba(0,0,0,1)] text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-3xl font-black text-white tracking-tight mb-4">Confirm Publish</h3>
