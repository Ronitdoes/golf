    ...
    <div className="flex min-h-screen bg-neutral-950 text-neutral-200">
      ...
      <aside className="hidden md:flex flex-col w-72 border-r border-white/5 bg-neutral-950 px-6 py-10 sticky top-0 h-screen shrink-0 overflow-y-auto custom-scrollbar">
        ...
        <div className="mt-auto pt-6">
           <Link href="/" className="flex items-center gap-3 px-2 py-4 text-[9px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-all group/exit">
              <div className="w-8 h-8 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-center group-hover/exit:bg-green-500/10 group-hover/exit:border-green-500/20 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-white/30 group-hover/exit:text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                </svg>
              </div>
              <span className="group-hover/exit:translate-x-1 transition-transform">Exit Dashboard</span>
           </Link>
        </div>
        ...
        <div className="border-t border-white/5 pt-10 px-2 flex items-center justify-between relative z-10 shrink-0">
           ...
           <form action={signOut}>
             <button type="submit" className="p-2.5 text-neutral-600 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all" title="Terminate Session">
      ...
      <main className="flex-1 flex flex-col relative w-full bg-neutral-950 overflow-x-hidden">
        ...
        <header className="md:hidden sticky top-0 flex items-center justify-between px-8 py-6 border-b border-white/5 bg-neutral-950/80 backdrop-blur-3xl shrink-0 z-50">
          <Link href="/dashboard" className="flex items-center">
            <span className="text-sm font-black text-white uppercase tracking-widest truncate max-w-[150px]">Player Dashboard</span>
          </Link>
          <div className="flex items-center gap-4">
