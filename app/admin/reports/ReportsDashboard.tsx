...
interface SubscriberStats { active: number; profit?: number; }
    ...
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         ...
         <StatCard 
            ...
            value={`£${charityStats.totalImpact.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} 
         ...
         <StatCard 
            ...
            value={`£${Number(subscriberStats.profit || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`} 
            icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>}
         />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="h-[300px] w-full relative z-10">
               {prizeStats.history.length > 0 ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center border border-white/5 border-dashed rounded-2xl">
                     <span className="text-white/20 font-black uppercase tracking-widest text-xs">No Allocation Data</span>
                  </div>
               )}
            </div>
         </motion.div>
         ...
         >
            ...
            <div className="h-[300px] w-full relative z-10">
               {charityStats.breakdown.length > 0 ? (
                  <div className="absolute inset-0">
                    <ResponsiveContainer width="100%" height="100%">
                       <PieChart>
                         <Pie
                         ...
                         <Tooltip 
                            ...
                            formatter={(value: any) => [`€${Number(value || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 'Accumulated']}
                         />
                       </PieChart>
                    </ResponsiveContainer>
                  </div>
                  ...
                  <div className="w-full h-full flex items-center justify-center border border-white/5 border-dashed rounded-2xl">
                     <span className="text-white/20 font-black uppercase tracking-widest text-xs">No Impact Data</span>
                  </div>
               )}
            </div>
